...
### 2.7 Equipment Management — `/v1/equipment` _(Mới từ dev)_
...
### RoomTypeEquipmentConfig _(Mới)_
...
### RoomEquipment _(Mới — chưa có endpoint)_
...
### EquipmentHistory _(Mới — chưa có endpoint)_
...
### InspectionEquipmentDetail _(Mới — chưa có endpoint)_
...
### 4.6 Quản lý Thiết bị (Equipment Management) _(Mới từ dev)_
...
const getAdminPasswordHash = async () => {
  ...
  if (!raw) {
    ...
    return false;
  } // Sentinel: disable admin login khi không có env
...
const deleteUser = async (id) => {
  ...
  if (student) {
    ...
    if (visitorCount > 0)
      throw new Error(`Cannot delete user: has ${visitorCount} visitor request(s).`);
    if (violationCount > 0)
      throw new Error(`Cannot delete user: has ${violationCount} violation report(s).`);
  }
...
| RoomEquipment             | Thiết bị thực tế trong phòng _(Category/Template/Config đã có endpoint)_ |
...
| Bug fix   | `src/services/ewUsage.service.js`                           | 2026-04-03 | Fix falsy check `meter_right \|\| meter_left` → dùng `meter_right != null ? meter_right : meter_left` (4 chỗ trong create/update/import). Fix trước: meter_right=0 fallback sai sang meter_left                                                                                                                    |
| Bug fix   | `src/services/ewUsage.service.js`                           | 2026-04-03 | Fix negative consumption: `updateEWUsage` + import — đổi `meter_right > 0` thành `meter_right > old_meter` để tránh consumption âm khi công tơ reset                                                                                                                                                              |
| Bug fix   | `src/services/ewUsage.service.js`                           | 2026-04-03 | `recalculate()`: fix duplicate invoice khi đã paid — query bất kể payment_status, skip nếu paid, reactivate nếu cancelled                                                                                                                                                                                         |
| Bug fix   | `src/services/ewUsage.service.js`                           | 2026-04-03 | `recalculate()`: fix rounding loss — dùng `Math.floor` + student cuối nhận phần dư thay vì `Math.round` cho tất cả                                                                                                                                                                                                 |
| Bug fix   | `src/services/ewUsage.service.js`                           | 2026-04-03 | `recalculate()`: bỏ `'approved'` khỏi contract status query (không tồn tại trong Contract enum), chỉ dùng `status: 'active'`                                                                                                                                                                                       |
| Bug fix   | `src/services/ewUsage.service.js`                           | 2026-04-03 | `recalculate()`: wrap mỗi student invoice creation trong try-catch — log error nhưng không crash toàn bộ flow; tránh mark is_billed khi có student fail                                                                                                                                                             |
| Bug fix   | `src/services/ewUsage.service.js`                           | 2026-04-03 | `getMyEWUsages()`: bỏ `'approved'` khỏi contract status query — chỉ dùng `status: 'active'`                                                                                                                                                                                                                        |
| Feature   | `src/services/ewUsage.service.js`                           | 2026-04-03 | Giá theo loại: electric=3000đ/kW, water=9000đ/m³ — thêm `PRICE_MAP` + `getPricePerUnit()`, thay hardcode 3000 toàn b�� (create/update/import)                                                                                                                                                                       |
| Feature   | `src/models/ewUsage.model.js`                               | 2026-04-03 | Pre-save hook: auto set `price_per_unit` theo type (water=9000, electric=3000) khi tạo mới hoặc đổi type                                                                                                                                                                                                           |
| Feature   | `src/services/ewUsage.service.js`                           | 2026-04-03 | Import: hỗ trợ cả điện (E) + nước (W) trong cùng file Excel; duplicate detection đổi từ block+type+date sang block+type+tháng (YYYY-MM)                                                                                                                                                                           |
| Feature   | `src/services/ewUsage.service.js`                           | 2026-04-03 | Import: validate chỉ số giảm — nếu meter mới < meter cũ → warning + consumption=0 (không reject, vẫn lưu); trả `warnings` count trong response                                                                                                                                                                    |
| Refactor  | `src/services/ewUsage.service.js`                           | 2026-04-03 | `resetMeter(id, body)`: đổi từ set=0 sang nhận `meter_right` bất kỳ (thay công tơ vật lý), consumption=0, is_billed=false. Xoá `resetAllMeters()`                                                                                                                                                                  |
| Refactor  | `src/controllers/ewUsage.controller.js`                     | 2026-04-03 | `resetMeter`: truyền `req.body`. Xoá `resetAllMeters` handler + export                                                                                                                                                                                                                                             |
| Refactor  | `src/routes/v1/ewUsage.route.js`                            | 2026-04-03 | Xoá route `PUT /reset-all`                                                                                                                                                                                                                                                                                          |
| Feature   | `src/services/ewUsage.service.js`                           | 2026-04-03 | `updateEWUsage` + `resetMeter`: validate chỉ cho phép thao tác trên record mới nhất của block+type. Bỏ cho phép đổi `date` khi update                                                                                                                                                                              |
...
| EW Usage     | ✅ 10 endpoints            | ✅      | ✅         | Hoạt động — meter tracking + recalculate    |
| Invoice      | ✅ 1 endpoint              | ✅      | ✅         | Hoạt động — student GET /invoices/my        |
